...
execute as @s[nbt={SelectedItem:{id:"minecraft:scute",tag:{BallistaAmmo:1b}}}] at @e[type=item_frame,tag=Ballista,nbt={ItemRotation:3b}] positioned ~ ~0.2 ~ as @e[type=armor_stand,tag=Ballista,tag=!Interacted,scores={wpz-phase=3,wpz-timer=0},distance=..0.1,limit=1,sort=nearest] at @s run function wpz:engines/ballista/interacts/load

execute at @e[type=item_frame,tag=Ballista,nbt={ItemRotation:3b}] positioned ~ ~0.2 ~ as @e[type=armor_stand,tag=Ballista,tag=!Interacted,distance=..0.1,limit=1,sort=nearest] at @s unless score @s wpz-timer matches 1.. run function wpz:engines/ballista/interacts/prep
